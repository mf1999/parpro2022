...
void boundarypsi(double* psi, int m, int n, int b, int h, int w)
{
	...
	for (i = index + b + 1; i <= b + w - 1; i+=stride)
	{
		psi[i * (m + 2) + 0] = (double)(i - b);
	}

	for (i = index + b + w; i <= m; i += stride)
	{
	...
	for (j = index + 1; j <= h; j+=stride)
	{
		psi[(m + 1) * (m + 2) + j] = (double)w;
	}

	for (j = index +  h + 1; j <= h + w - 1; j+=stride)
	{
...
__global__
void deltasq(double* newarr, double* oldarr, int m, int n, double*dsq)
{
	int i, j;
	...
	for (i = index + 1; i <= m; i+=stride)
	{
		for (j = 1; j <= n; j++)
		{
			...
			//if (tmp > 0.0) printf("i: %d, j: %d - %f", tmp);
...
int main(int argc, char** argv)
{
	...
	cudaMallocManaged(&psitmp, N * sizeof(double));
	cudaMemset(psi, 0, N * sizeof(double));
	//zero the psi array
	/*
	for (i = 0; i < m + 2; i++) {
		for (j = 0; j < n + 2; j++) {
	...
	*/
	...
	boundarypsi<<<numBlocks, blockSize>>>(psi, m, n, b, h, w);
	...
	for (iter = 1; iter <= numiter; iter++) {
		...
		jacobistep<<<numBlocks, blockSize>>>(psitmp, psi, m, n);
		cudaDeviceSynchronize();
		...
		if (checkerr || iter == numiter) {
			...
			//kernelized
			double* dsq;
			cudaMallocManaged(&dsq, sizeof(double));
			cudaMemset(dsq, 0, sizeof(double));
			deltasq << < numBlocks, blockSize>>>(psitmp, psi, m, n, dsq);
			cudaDeviceSynchronize();

			cudaMemcpy(error, dsq, sizeof(double), cudaMemcpyDeviceToHost);
		...
		//copy back
		cudaMemcpy(psi, psitmp, N * sizeof(double), cudaMemcpyDeviceToDevice);
		/*
		for (i = 1; i <= m; i++) {
			for (j = 1; j <= n; j++) {
